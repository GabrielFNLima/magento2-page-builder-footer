define([
    ...
    /**
     * Bind events to add empty Footer Link when Footer added and reinitialize accordion when Footer item added
     */
    Preview.prototype.bindEvents = function bindEvents() {
        ...
        events.on("footer-item:dropAfter", function (args) {
            if (args.id === self.contentType.id && self.contentType.children().length === 0) {
                self.addFooterLink();
            }
        });

        events.on("footer-link:renderAfter", (args) => {
     ...
     * Add Footer Link
     */
    Preview.prototype.addFooterLink = function () {
        var self = this;
        createContentType(
            pageBuilderConfig.getContentTypeConfig("footer-link"),
            this.contentType,
            this.contentType.stageId,
            {
                question: $t("Link ") + (self.contentType.children().length + 1)
            }
